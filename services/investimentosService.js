...
const validationRequest = (body) => {
  ...
  if (!codAtivo) {
    throw new
    HttpError(400, 'por favor identificar qual ativo deseja comprar/vender');
  }
  if (!qtdeAtivo) {
    throw new
    HttpError(
  ...
  if (qtdeAtivo < 0) {
    throw new HttpError(400, 'quantidade não pode ser negativa');
  }
};
...
const validationSellOrder = (qtdeAtivo, seller, sellerAsset) => {
  ...
  if (!sellerAsset) {
    throw new HttpError(404, 'voce nao possui esse ativo para vende-lo');
  }

  const assetsAvailableForSelling = parseFloat(sellerAsset.qtdeAtivo);
  if (qtdeAtivo > assetsAvailableForSelling) {
    throw new HttpError(412, 'voce nao possui essa quantidade para venda');
  }
};
...
const executeBuyOrder = async (qtdeAtivo, buyer, requestedAsset) => {
  ...
  const updateFunds = await models
      .countBalanceMovement(
          {codCliente, valor: purchasePrice, transactionType: 'compra'},
      );
...
const executeSellOrder = async (qtdeAtivo, seller, sellerAsset) => {
  ...
  const sellOrder = await models
      .buyOrSellRequest({codCliente, codAtivo, qtdeAtivo: qtdeAtivoSold});
  if (!sellOrder.insertId) throw new HttpError(500, 'alguma coisa deu errado!');

  const updateFunds = await models
      .countBalanceMovement(
          {codCliente, valor: sellValue, transactionType: 'venda'},
      );
  if (!updateFunds.insertId) {
